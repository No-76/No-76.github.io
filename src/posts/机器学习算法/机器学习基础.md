...
### 正则化
$$
L1正则化:J(w, b)=\frac{1}{2 m} \sum_{i=1}^m\left(f_{w, b}\left(x^{(i)}\right)-y^{(i)}\right)^2+λ * ||w||₁
$$  
$$
L2正则化:J(w, b)=\frac{1}{2 m} \sum_{i=1}^m\left(f_{w, b}\left(x^{(i)}\right)-y^{(i)}\right)^2+λ * ||w||₂²
$$  
...
**卷积层**
$$
卷积:s(t)=\int_{-\infty}^{\infty} f(a) * g(t-a) \mathrm{d} a
$$
